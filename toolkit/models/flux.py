...
def new_device_to(self: FluxTransformer2DModel, *args, **kwargs):
    ...
    # Check if blocks have split devices, otherwise use the main device
    for block in self.transformer_blocks:
        target_device = getattr(block, '_split_device', device)
        block.to(target_device, *args, **kwargs)
...
def add_model_gpu_splitter_to_flux(
    ...
    if not torch.cuda.is_available():
        print("Warning: CUDA not available. GPU splitting will be skipped.")
        return

    cuda_device_count = torch.cuda.device_count()
    if cuda_device_count == 0:
        print("Warning: No CUDA devices found. GPU splitting will be skipped.")
        return

    gpu_id_list = [i for i in range(cuda_device_count)]
    ...
    transformer.single_transformer_blocks[-1]._split_output_device = torch.device(f"cuda:0")
