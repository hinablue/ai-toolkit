...
def broadcast_and_multiply(tensor, multiplier):
    # Handle AffineQuantizedTensor by dequantizing first
    from torchao.dtypes.affine_quantized_tensor import AffineQuantizedTensor
    if isinstance(tensor, AffineQuantizedTensor):
        tensor = tensor.dequantize()
    if isinstance(multiplier, AffineQuantizedTensor):
        multiplier = multiplier.dequantize()
...
def add_bias(tensor, bias):
    ...
    if isinstance(tensor, AffineQuantizedTensor):
        tensor = tensor.dequantize()
    if isinstance(bias, AffineQuantizedTensor):
