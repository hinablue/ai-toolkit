...
const GpuMonitor: React.FC = () => {
  ...
  const content = useMemo(() => {
    ...
    if (!gpuData.hasMps) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <strong className="font-bold">No MPS GPU detected!</strong>
          <span className="block sm:inline"> MPS is not available on this system.</span>
    ...
    if (gpuData.gpus.length === 0) {
      return (
        <div className="bg-yellow-900 border border-yellow-700 text-yellow-300 px-4 py-3 rounded relative" role="alert">
          <span className="block sm:inline">No GPUs found, but MPS is available.</span>
        </div>
      );
    }
